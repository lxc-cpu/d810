...
3. Normalized Gaussian Wasserstein Distance(支持v11-detect、v11-seg、v11-pose)[论文链接](https://arxiv.org/abs/2110.13389)
    ...
    以上这两可以配合使用,也可以单独使用.(视频教程:项目视频百度云链接-20251122版本更新说明)
...
5. Gaussian Combined Distance(支持v11-detect、v11-seg、v11-pose)[论文链接](https://arxiv.org/pdf/2510.27649)

    在Loss中使用:
        在ultralytics/utils/loss.py中的BboxLoss class中的__init__函数里面设置self.gcd_loss为True.  
        比例系数调整self.iou_ratio, self.iou_ratio代表iou的占比,(1-self.iou_ratio)为代表gcd的占比.  
    在TAL标签分配中使用:
        在ultralytics/utils/tal.py中的def iou_calculation函数中进行更换即可.
    以上这两可以配合使用,也可以单独使用.(视频教程:项目视频百度云链接-20251122版本更新说明)